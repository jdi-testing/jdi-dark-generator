...
import static com.epam.jdi.generator.v2.core.JavaCodegenConstantsJDI.DATE_LIBRARY_DESC;
import static com.epam.jdi.generator.v2.core.JavaCodegenConstantsJDI.SERIALIZATION_LIBRARY_DESC;
...
@Option(name = {
        "-o",
        "--output"
}, title = "output directory", description = "where to write the generated files. Defaults to generated-code folder")
private String outputDir;
@Option(name = {
        "-p",
        "--invoker-package"
}, title = "invoker package", description = "root package for generated code")
...
@Option(name = {"--serialization-library"}, title = "serialization library", description = SERIALIZATION_LIBRARY_DESC)
private String serializationLibrary;
@Option(name = {"--date-library"}, title = "date library", description = DATE_LIBRARY_DESC)
private String dateLibrary;
@Option(name = {
        "-a",
        "--auth"
}, title = "authorization", description = "adds authorization headers when fetching the swagger definitions remotely. Pass in a URL-encoded string of name:header with a comma separating multiple " +
        "values")
private String auth;
@Option(name = {
        "-c",
...
@Option(name = {"--codegen-arguments"}, title = "codegen arguments", description = "codegen arguments")
...
@Inject
protected HelpOption<Generate> help;
...
//        help.showHelp(new CliCommandUsageGenerator());
    ...
    if (configurator.getInputSpecURL() == null) {
        help.showHelp();
        throw new IllegalArgumentException(String.format("%n%nInput specification must be specified either with command line switch or in supplied config file.%n"));
    }
...
private void overrideConfigWithCLIArguments(CodegenConfiguratorJDI configurator) {
    ...
    if (isNotEmpty(dateLibrary)) {
        configurator.setDateLibrary(dateLibrary);
    }
    ...
    if (useOas2 != null) {
        configurator.setUseOas2(useOas2);
    }
